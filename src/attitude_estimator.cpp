...
void AttitudeEstimator::updateQy(double accX, double accY, double accZ, double magX, double magY, double magZ)
{
	...
	// Expected gravity magnitude (tune this!)
	constexpr double g = 9.81;
	constexpr double g_tol = 2.0; // m/s^2 tolerance

	double accNorm = sqrt(naccsq);
	bool accValid = fabs(accNorm - g) < g_tol;


	if(naccsq < ACC_TOL_SQ || !accValid)
	{
		// Reject acc → do NOT tilt-correct this cycle
		m_Qy[0] = m_Qhat[0];
		m_Qy[1] = m_Qhat[1];
		m_Qy[2] = m_Qhat[2];
		m_Qy[3] = m_Qhat[3];
